 ...
 * (the <img> hides on error), so this always returns paths keyed by slug. */
export function speciesPortraitFor(slug?: string | null): { m: string; f: string } {
  const s = norm(slug).replace(/\s+/g, "-");
  return { m: `/assets/species/${s}-m.webp`, f: `/assets/species/${s}-f.webp` };
}
